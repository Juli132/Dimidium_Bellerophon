...
document.addEventListener("DOMContentLoaded", () => {
  ...
  downloadjup.addEventListener("click", () => {
    ...
    if (!jupContent && !confirm("The Jupitore file is empty. Save anyway?"))
      return;
...
window.highlightGCode = function (rawGcode) {
  ...
  formattedCode = formattedCode.replace(
    /^(\[.*?\])/gm,
    `<span style="color: ${colors.header}">$1</span>`,
  );
  formattedCode = formattedCode.replace(
    /^(gcode:)/gm,
    `<span style="color: ${colors.header}">$1</span>`,
  ...
  formattedCode = formattedCode.replace(
    /\b([GM]\d+)\b/g,
    `<span style="color: ${colors.command}">$1</span>`,
  );
  ...
  formattedCode = formattedCode.replace(
    /\b([XYZEFSP]|SPEED|TIMEOUT|ADVANCE)(=|\s*)?(-?\d+(\.\d+)?)/g,
