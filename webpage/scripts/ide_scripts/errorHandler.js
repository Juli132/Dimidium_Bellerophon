...
const keywords = new Set([
    "M.TITLE", "M.END", "HOME", "MOVE", "LEVEL", "M.CALL", "IF", "ENDIF", 
    "PAUSE", "RESPOND", "RESUME", "SET_HEATER_TEMPERATURE", "WAITFORTEMP", 
    "DWELL", "MOVETO", "ABSOLUTE", "RELATIVE", "TIMEOUT_SET", 
    "RELATIVEEXTRUSION", "LOADBEDMESH", "SETPRESSUREADVANCE", 
    "RESETEXTRUDER", "BEDMESHCALIBRATE", "PROBECALIBRATE", "COOLDOWN", 
    "SETSPEED", "SETFAN", "PRINTFILE", "MSG", "REPEAT", "BREPEAT", "END",
    "LEFT", "RIGHT", "CENTER", "UP", "DOWN", "BED", "EXTRUDER", "CHAMBER"
]);
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    compileBtn.addEventListener("click", async () => {
        ...
        logMessage(log, `Starting Bellerophon build... [${new Date().toLocaleTimeString()}]`, "system");
        ...
        try {
            ...
            if (semanticErrors.length > 0) {
                semanticErrors.forEach(err => {
                    logMessage(log, `Line ${err.line}: ${err.message}`, err.type === "error" ? "error" : err.type === "warning" ? "warning" : "hint");
   ...
   profile: profile
       ...
       if (data.success) {
              ...
              // added this here
                ...
                logMessage(log, `Build failed: ${data.error || "Unknown compiler error"}`, "error");
            }
...
if (code.length > 100000) { 
        logMessage(document.getElementById("console-log"), "Large Script Detected! Skipping real-time syntax highlighting for performance.", "system");
        return []; 
    }
        ...
        lines.forEach((line, idx) => {
            ...
            if (/^M\.title\b/i.test(trimmed)) {
                if (insideMacro) {
                    macroErrors.push({
                        line: macroStartLine,
                        message: "Previous macro must end with M.end before starting a new macro",
                        type: "error"
                    });
                }
                insideMacro = true;
            ...
            if (!insideMacro) {
                macroErrors.push({
                    line: lineNum,
                    message: "Command found outside of a macro. Each macro must start with M.title and end with M.end",
                    type: "error"
                });
            }
        });
    ...
    if (["EXTRUDER", "BED", "FAN"].includes(firstWord)) {
        ...
        tokens.forEach(t => {
            ...
            if (t.name === "UNRECOGNIZED") {
                errors.push({ line: lineNum, message: `Unrecognized token: ${text}`, type: "error" });
            }
